<template>
    <el-container>
        <div style="width:100%">
            <div class="modular_header">
                <el-header class="recommend-song">歌单推荐</el-header>
                <el-main class="recommend-song-list">
                    <el-header>
                        <el-row type="flex" class="row-bg row-first" justify="center">
                            <el-col :span="2">
                                <div :class="{'grid-content':isactive}">为你推荐</div>
                            </el-col>
                            <el-col :span="2">
                                <div :class="{'grid-content':isactive}">情歌</div>
                            </el-col>
                            <el-col :span="2">
                                <div :class="{'grid-content':isactive}">节奏控</div>
                            </el-col>
                            <el-col :span="2">
                                <div :class="{'grid-content':isactive}">网络歌曲</div>
                            </el-col>
                            <el-col :span="2">
                                <div :class="{'grid-content':isactive}">官方歌曲</div>
                            </el-col>
                            <el-col :span="2">
                                <div :class="{'grid-content':isactive}">经典</div>
                            </el-col>
                        </el-row>
                    </el-header>
                    <el-main>
                        <el-carousel
                            trigger="click"
                            height="330px"
                            :interval="5000"
                            @change="recommendonchange"
                            :autoplay="false"
                        >
                            <el-carousel-item
                                v-for="(item,index) in pageIndex"
                                :key="index"
                                class="carousel_css"
                            >
                                <div v-for="(item,index) in currentPagemusicdata" :key="index">
                                    <!-- <div class="play_img">
                                        <img src="../../../assets/playicon.png" alt="">
                                    </div> -->
                                    <div class="carousel_itme">
                                        <a :href="item.href">
                                            <img :src="item.cover" alt srcset width="230" height="230" @click="getsingermusic(item)"/>
                                        </a>
                                    </div>
                                    <span class="font_14 font_weight">{{item.title}}</span>
                                    <div>
                                        <span class="font_14 font_color_ccc ">播放量：{{Math.ceil(item.listen_num/1000)/10}}万</span>
                                    </div>
                                </div>
                            </el-carousel-item>
                        </el-carousel>
                    </el-main>
                </el-main>
            </div>
            <div class="modular_header">
                <el-header class="recommend-song">新歌首发</el-header>
                <el-main class="recommend-song-list">
                    <el-header>
                        <el-row type="flex" class="row-bg" justify="center">
                            <el-col :span="2">
                                <div :class="{'grid-content':isactive}">最新</div>
                            </el-col>
                            <el-col :span="2">
                                <div :class="{'grid-content':isactive}">内地</div>
                            </el-col>
                            <el-col :span="2">
                                <div :class="{'grid-content':isactive}">港台</div>
                            </el-col>
                            <el-col :span="2">
                                <div :class="{'grid-content':isactive}">韩国</div>
                            </el-col>
                            <el-col :span="2">
                                <div :class="{'grid-content':isactive}">欧美</div>
                            </el-col>
                            <el-col :span="2">
                                <div :class="{'grid-content':isactive}">日本</div>
                            </el-col>
                        </el-row>
                    </el-header>
                    <el-main>
                        <el-carousel
                            trigger="click"
                            height="400px"
                            :interval="5000"
                            @change="nwemusiconchange"
                            :autoplay="false"
                        >
                            <el-carousel-item
                                v-for="(item,index) in newpageIndex"
                                :key="index"
                                class="newcarousel_css"
                            >
                                <div v-for="(item,index) in newCurrentmusicdata" :key="index">
                                    <span>
                                        <img :src="item.url" width="86" height="86" @click="playmusic(item)"/>
                                    </span>
                                    <span>
                                        <span class="singer-musicname font_14">{{item.album_name}}</span>
                                        <span class="singer-musicname font_14 font_color_ccc">{{item.singers[0].singer_name}}</span>
                                    </span>
                                  
                                    <span class="time-musicname font_14 font_color_ccc">{{item.public_time}}</span>
                                </div>
                            </el-carousel-item>
                        </el-carousel>
                    </el-main>
                </el-main>
            </div>
        </div>
    </el-container>
</template>

<script>
import $jquery from 'jquery'
const params = {
    filter: true,
    loop: true,
    target: "auto"
};
let player = new QMplayer({
    params
});
export default {
    data() {
        return {
            musiclistdata: [],//歌单推荐的总数居
            currentPagemusicdata: [],//歌单推荐的分页数据
            currentPage: 1,
            pageSize: 5,
            pageIndex: 1,
            newmusiclist: [],//新歌首发的总数据
            newCurrentmusicdata:[],//新歌首发的分页数据
            newpageSize:9,
            newcurrentPage:1,
            newpageIndex:1,
            newmusicimg:[],//新歌推荐获取的img
            isactive:false,
        };
    },
    mounted() {
        this.getmusiclist();
    },
    methods: {
        /**
        * @description 歌单推荐的列表
        * @param {} 
        * @return 
        * @author lisheng
        **/
        getmusiclist() {
            let url = `/musicall/cgi-bin/musicu.fcg?g_tk=1742895503&jsonpCallback=recom6880496722127585&loginUin=2034969809&hostUin=0&format=jsonp&inCharset=utf8&outCharset=utf-8&notice=0&platform=yqq&needNewCode=0&data=%7B%22comm%22%3A%7B%22ct%22%3A24%7D%2C%22category%22%3A%7B%22method%22%3A%22get_hot_category%22%2C%22param%22%3A%7B%22qq%22%3A%22%22%7D%2C%22module%22%3A%22music.web_category_svr%22%7D%2C%22recomPlaylist%22%3A%7B%22method%22%3A%22get_hot_recommend%22%2C%22param%22%3A%7B%22async%22%3A1%2C%22cmd%22%3A2%7D%2C%22module%22%3A%22playlist.HotRecommendServer%22%7D%2C%22playlist%22%3A%7B%22method%22%3A%22get_playlist_by_category%22%2C%22param%22%3A%7B%22id%22%3A8%2C%22curPage%22%3A1%2C%22size%22%3A40%2C%22order%22%3A5%2C%22titleid%22%3A8%7D%2C%22module%22%3A%22playlist.PlayListPlazaServer%22%7D%2C%22new_song%22%3A%7B%22module%22%3A%22QQMusic.MusichallServer%22%2C%22method%22%3A%22GetNewSong%22%2C%22param%22%3A%7B%22type%22%3A0%7D%7D%2C%22new_album%22%3A%7B%22module%22%3A%22music.web_album_library%22%2C%22method%22%3A%22get_album_by_tags%22%2C%22param%22%3A%7B%22area%22%3A7%2C%22company%22%3A-1%2C%22genre%22%3A-1%2C%22type%22%3A-1%2C%22year%22%3A-1%2C%22sort%22%3A2%2C%22get_tags%22%3A1%2C%22sin%22%3A0%2C%22num%22%3A40%2C%22click_albumid%22%3A0%7D%7D%2C%22toplist%22%3A%7B%22module%22%3A%22music.web_toplist_svr%22%2C%22method%22%3A%22get_toplist_index%22%2C%22param%22%3A%7B%7D%7D%2C%22focus%22%3A%7B%22module%22%3A%22QQMusic.MusichallServer%22%2C%22method%22%3A%22GetFocus%22%2C%22param%22%3A%7B%7D%7D%7D`;
            this.$axios.get(url).then(res => {
                // console.log(res); 
                this.musiclistdata = res.data.recomPlaylist.data.v_hot;//歌单推荐的请求数据
                this.getCurrentPageData();//
                let num = parseInt(this.musiclistdata.length / this.pageSize);
                this.pageIndex = num;
                
                this.newmusiclist = res.data.new_album.data.list//新歌首发的数据 (album_mid 歌曲ID)(album_name歌曲名字)(singers:[歌手]) 
                // console.log(this.newmusiclist);
                this.getnewmusicCurrentPageData()
                let newnum = parseInt(this.newmusiclist.length / this.newpageSize)
                this.newpageIndex = newnum
                
            });
        },
        /**
        * @description 推荐歌单分页设置
        * @param {} 
        * @return 
        * @author lisheng
        **/
        getCurrentPageData() {
            let begin = (this.currentPage - 1) * this.pageSize;
            let end = this.currentPage * this.pageSize;
            this.currentPagemusicdata = this.musiclistdata.slice(begin, end);
            this.currentPagemusicdata.forEach((item,index) => {
                let href = `https://y.qq.com/n/yqq/playsquare/${item.content_id}.html#stat=y_new.index.playlist.pic`
                this.$set(this.currentPagemusicdata[index],'href',href)
            })
        },
        /**
        * @description 推荐歌单轮播改变时候的函数
        * @param {} 
        * @return 
        * @author lisheng
        **/
        recommendonchange(a, i) {
            this.currentPage = a + 1;
            this.getCurrentPageData();
        },
        /**
        * @description 新歌首发的分页设置
        * @param {} 
        * @return 
        * @author lisheng
        **/
        getnewmusicCurrentPageData(){
            let begin = (this.newcurrentPage - 1) * this.newpageSize;
            let end = this.newcurrentPage * this.newpageSize;
            this.newCurrentmusicdata = this.newmusiclist.slice(begin,end)    
            //遍历得到对应的专辑的图片   
            this.newCurrentmusicdata.forEach((itme,index) => {
                let url = `https://y.gtimg.cn/music/photo_new/T002R300x300M000${itme.album_mid}.jpg?max_age=2592000`
                this.$set(this.newCurrentmusicdata[index],'url',url)
            })          
        },
        /**
        * @description 新歌首发轮播改变的数据
        * @param {} 
        * @return 
        * @author lisheng
        **/
       nwemusiconchange(a,i){
           this.newcurrentPage = a + 1
           this.getnewmusicCurrentPageData()
       },
        // changeColor(e){
        //     // this.isactive = true 
        //     console.log(e.currentTarget.parentElement.children); 
        //     $jquery(e.target).css({"color":"#67C23A"})    
        //     // $jquery(e.target.parentElement)
        // }
        player(musicID){
            player.play(musicID)
        },
        playmusic(row){
            console.log(row)
            // this.player(row.album_mid)
            // this.player("003NGKTc2tabpb")
            this.player("000niUQw0FPijO")
        },
    },
};
</script>

<style scoped>
.recommend-song {
    padding: 40px;
    font-size: 30px;
    width: 100%;
    font-weight: 700;
    letter-spacing: 15px
}
.recommend-song-list {
    width: 100%;
}
.carousel_css {
    display: flex;
}
.carousel_css > div {
    flex: 1;
    text-align: left;
}
.newcarousel_css{
    display: flex;
    flex-wrap: wrap;
}
.newcarousel_css >div {
    display: inline-block;
    width: 33%;
    text-align: left;
    position: relative;
}
/* .play_img{
    position: absolute;
    width: 230px;
    height: 230px;
    background-color: rgba(243, 234, 234, 0.6);
}
.play_img > img{
    margin:50% 0 0 50%;
    transform: translate(-50%,-50%)
} */
.newcarousel_css >div >span {
    display: inline-block;
    vertical-align: middle
}

.modular_header {
    width: 100%;
    display: block;
    background: linear-gradient(rgb(255, 255, 255), rgb(240, 237, 237));
}
.font_14{
    font-size: 14px;  
}
.font_weight{
    font-weight: 700;
}
.carousel_itme{
    margin-bottom: 5px
}
.font_color_ccc{
    color: rgb(179, 178, 178);
}
.singer-musicname{
    display: block;
    padding-left: 10px
}
.time-musicname{
    position: absolute;
    right: 10px;
    top:43px;
    transform: translateY(-50%)
}
.row-first > .el-col:hover{
    color: #67C23A;
}
.row-first > .el-col{
    padding: 5px;
}
.grid-content{
    color: #67C23A;
}
</style>